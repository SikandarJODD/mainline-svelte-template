...
>
	<div class="flex items-center justify-between px-6 py-3">
		<a href="/" class="flex shrink-0 items-center gap-2">
			<img src="/logo.svg" alt="logo" width="94" height="18" class="dark:invert" />
		</a>
			...
			<NavigationMenu.List>
				{#each ITEMS as link}
					{#if link.dropdownItems}
						<NavigationMenu.Item>
							<NavigationMenu.Trigger class="data-[state=open]:bg-accent/50 bg-transparent! px-1.5">
								{link.label}
						...
						<NavigationMenu.Item>
							<NavigationMenu.Link >
								<a
									href={link.href}
									class={cn(
										'text-muted-foreground hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground group inline-flex h-9 w-max items-center justify-center rounded-md px-4 py-2 text-sm font-medium transition-colors focus:outline-none disabled:pointer-events-none disabled:opacity-50',
										$page.url.pathname === link.href && 'bg-accent/50 text-accent-foreground'
									)}
								>
									{link.label}
								</a>
			...
			{#each ITEMS as link}
				{#if link.dropdownItems}
					<div class="py-4 first:pt-0 last:pb-0">
						<button
							onclick={() => (openDropdown = openDropdown === link.label ? null : link.label)}
